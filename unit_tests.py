import unittest
from engine import GameState, Cube_Decision, Move, WHITE, RED
...
class TestEngine(unittest.TestCase):
    ...
    def test_all_decisions(self):
        ...
        all_decision_options = [
            decision_option.decision_type for decision_option in r.get_all_decision_options()]
        self.assertEqual(all_decision_options, ['no double'])

        r = GameState()
        setattr(r, 'turn_number', 1)
        all_decision_options = set([
            decision_option.decision_type for decision_option in r.get_all_decision_options()])
        self.assertEqual(all_decision_options, set(['double', 'no double']))

        r = GameState()
        setattr(r, 'turn_number', 1)
        setattr(r, 'crawford', True)
        all_decision_options = [
            decision_option.decision_type for decision_option in r.get_all_decision_options()]
        self.assertEqual(all_decision_options, ['no double'])
        ...
        r = GameState()
        setattr(r, 'is_white_turn', True)
        setattr(r, 'score', {WHITE: 6, RED: 0})
        setattr(r, 'match_to', 7)
        setattr(r, 'turn_number', 1)
        all_decision_options = [
            decision_option.decision_type for decision_option in r.get_all_decision_options()]
        self.assertEqual(all_decision_options, ['no double'])

        # need to test what happens when you make a decision
